...
def create_cls_model(num_labels):
    ...
    # 词窗大小分别为3,4,5
    cnn1 = Conv1D(312, 3, padding='same', strides=1, activation='relu')(cls_layer)
    cnn1 = MaxPooling1D(3, 3, padding='same')(cnn1)
    cnn2 = Conv1D(312, 4, padding='same', strides=1, activation='relu')(cls_layer)
    cnn2 = MaxPooling1D(3, 3, padding='same')(cnn2)
    cnn3 = Conv1D(312, 5, padding='same', strides=1, activation='relu')(cls_layer)
    cnn3 = MaxPooling1D(3, 3, padding='same')(cnn3)

    # 合并三个模型的输出向量
    cnn = concatenate([cnn1, cnn2, cnn3], axis=-1)
    flat = Flatten()(cnn)
    drop = Dropout(0.2)(flat)
    p = Dense(num_labels, activation='sigmoid')(drop)
